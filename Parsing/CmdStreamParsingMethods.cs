    ...
    public void FixRetriggerCommands()
    {
        ...
        void FixRetriggerCmds(List<FurnaceCommand> cmdList)
        {
            ...
            for(var i = 1; i < noteCmdChLen; i++) {
                var curCmd        = cmdList[i];
                var prevNoteOnCmd = GetFirstCertainCmd(cmdList, i, cmd => cmd.CmdType.Equals("NOTE_ON"), direction: "backward", isCmdFound: out _);
                var nextNoteOnCmd = GetFirstCertainCmd(cmdList, i, cmd => cmd.CmdType.Equals("NOTE_ON"), direction: "forward", isCmdFound: out _);
